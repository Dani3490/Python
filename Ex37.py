...
while partidaNova == ("s") or partidaNova == ("si"):
    ...
    haGuanyat = trobada(nTalaiot)
    
    if haGuanyat:
        punts += 1
        print(f"Has guanyat! Ara tens {punts} punt(s)!")
    else:
        print(f"\nHas perdut! Puntuació final: {punts} punt(s)")
        punts = 0  # Reiniciem els punts després de perdre
    
    partidaNova = input("\nVols tornar a menjar (jugar)? Introdueixi si o no: ")
    print("\n")

print(f"Gràcies per jugar! Puntuació final: {punts} punt(s)")